...
import { handleGetCreatorBalance, handleGetCreatorEarnings, handleCreatorWithdraw, handleGetCreatorPayouts, handleSetPayoutMethod, handleGetPayoutMethods, handleAdminListPayouts, handleAdminApprovePayout, handleAdminRejectPayout, handleAdminChargeback, handleShopBuy, handleShopRefund, handleShopPurchases, } from './routes/payout';
...
function createBattle(hostRoomId, hostUserId, hostName) {
    const session = {
...
function joinBattle(roomId, opponentUserId, opponentName) {
...
function addBattleScoreForTarget(roomId, target, points) {
    const session = battles.get(roomId);
...
function broadcastBattleState(roomId, session) {
    if (session.endsAt > 0) {
    ...
    try {
        switch (event) {
            ...
            case 'battle_join': {
                ...
                const battleSession = joinBattle(client.roomId, client.userId, data.opponentName || client.displayName);
                if (!battleSession) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function sendToUser(roomId, userId, event, data) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function broadcastToRoom(roomId, event, data, exclude) {
    const room = rooms.get(roomId);
    if (!room)
        return;
    let message;
