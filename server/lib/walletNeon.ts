...
/** If user has no Neon row yet, copy current file wallet balance once (migration / hybrid). */
export async function neonEnsureBalanceFromFile(userId: string): Promise<void> {
  const pool = getPool();
  if (!pool || !userId) return;
  try {
    const r = await pool.query(
      `SELECT 1 FROM elix_wallet_balances WHERE user_id = $1 LIMIT 1`,
      [userId],
    );
    if (r.rows.length > 0) return;
    const { getWalletBalance } = await import("./walletStore.js");
    const fileBal = Math.max(0, Math.floor(getWalletBalance(userId)));
    if (fileBal <= 0) return;
    await pool.query(
