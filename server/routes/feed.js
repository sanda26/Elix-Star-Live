...
const TRENDING_CACHE_TTL = 30000;
const viewRateLimit = new Map();
const RATE_LIMIT_WINDOW = 60000;
const RATE_LIMIT_MAX_VIEWS = 120;
function computeScore(s) {
    return ((s.total_watch_time || 0) * SCORE_WEIGHTS.watch_time +
...
function checkRateLimit(key) {
    ...
    if (entry.count >= RATE_LIMIT_MAX_VIEWS)
        return false;
    entry.count++;
    return true;
}
...
async function getTrendingVideos(limit) {
    if (!db)
        return [];
    const now = Date.now();
...
async function getFollowingVideoIds(userId) {
    if (!db)
        return [];
...
async function getUserInterests(userId) {
    ...
    const map = new Map();
    data?.forEach((d) => map.set(d.category, d.weight));
    return map;
}
...
async function getLikedVideoCategories(userId) {
    if (!db)
        return [];
    // Use likes table
...
function personalizeAndRank(videos, followingIds, interests, watchedIds, notInterestedIds, likedCategories) {
    ...
    const scored = videos
        ...
        if (userId && followingSet.has(userId))
            score += 200;
        const cat = v.category;
        if (cat && interests.has(cat))
            score += interests.get(cat) * 50;
        if (cat && categoryBoost.has(cat))
            score += categoryBoost.get(cat) * 30;
...
function formatVideoForClient(v, likedSet, followingSet) {
    const u = v.user;
...
export async function handleForYouFeed(req, res) {
    try {
        if (!db) {
        ...
        if (userId) {
            const [followingIds, interests, watchedIds, notInterestedIds, likedCategories, userLikes, userFollowing] = await Promise.all([
                ...
                db.from('likes').select('video_id').eq('user_id', userId).then(r => r.data?.map((d) => d.video_id) || []),
                db.from('followers').select('following_id').eq('follower_id', userId).then(r => r.data?.map((d) => d.following_id) || []),
            ]);
            ...
            ranked.sort((a, b) => b._feedScore - a._feedScore);
        }
...
export async function handleTrackView(req, res) {
    try {
        if (!db)
            return res.status(503).json({ error: 'Database not configured' });
        const userId = await getUserId(req);
...
export async function handleTrackInteraction(req, res) {
    try {
        if (!db)
            return res.status(503).json({ error: 'Database not configured' });
        const userId = await getUserId(req);
        ...
        if (type === 'like') {
            ...
            if (existing.data && existing.data.length > 0) {
                ...
                if (r.data)
                    await db.from('videos').update({ likes: Math.max(0, (r.data.likes || 0) - 1) }).eq('id', videoId);
            }
            else {
                await db.from('likes').insert({ user_id: userId, video_id: videoId });
                const r = await db.from('videos').select('likes').eq('id', videoId).single();
                if (r.data)
                    await db.from('videos').update({ likes: (r.data.likes || 0) + 1 }).eq('id', videoId);
            }
        }
...
async function updateVideoScore(videoId) {
    if (!db)
        return;
    try {
        ...
        const totalWatchTime = watchTimeRes.data?.reduce((sum, r) => sum + (r.watch_time_seconds || 0), 0) || 0;
...
async function updateUserInterests(userId, videoId) {
    if (!db)
        return;
    try {
...
export async function handleGetVideoScore(req, res) {
    try {
        if (!db)
            return res.status(503).json({ error: 'Database not configured' });
