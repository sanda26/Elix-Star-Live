...
async function verifyAppleReceipt(transactionId) {
    ...
    try {
        ...
        const resp = await fetch(`${baseUrl}/inApps/v1/transactions/${transactionId}`, { headers: { Authorization: `Bearer ${jwt}` } });
        if (!resp.ok) {
        ...
        if (parts.length === 3) {
            const txPayload = JSON.parse(Buffer.from(parts[1], 'base64url').toString());
            return {
                valid: true,
...
// --- Verify Purchase ---
export async function handleVerifyPurchase(req, res) {
    if (req.method !== 'POST')
        return res.status(405).send('Method not allowed');
    const authHeader = req.headers.authorization;
