...
async function classifyImageWithAI(imageBase64) {
...
If the image shows only safe/neutral content (including smoking, drinking, or person in car as passenger), set "flagged": false and "category": null, "severity": null.`;
    try {
        ...
        if (!parsed.flagged)
            return { flagged: false };
        if (parsed.category && !DANGEROUS_CATEGORIES.includes(parsed.category))
            return { flagged: false };
        return {
            flagged: true,
...
export async function handleLiveModerationCheck(req, res) {
    if (req.method !== 'POST')
    ...
    const logEntry = (kind, category, severity, action_taken, details) => {
