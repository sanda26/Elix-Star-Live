...
import { listShopItems, createShopItem, type ShopItemRow } from "../lib/shopStore";
...
function enrichSeller(row: ShopItemRow) {
...
export function handleListShopItems(req: Request, res: Response) {
  if (req.method !== "GET") {
...
export function handleCreateShopItem(req: Request, res: Response) {
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }
