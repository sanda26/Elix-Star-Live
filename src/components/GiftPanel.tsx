...
// This file is locked to preserve the z-index and layout of the gift panel.
...
export function GiftPanel({ onSelectGift, userCoins, onRechargeSuccess }: GiftPanelProps) { 
  ...
  return ( 
    <div ref={panelRef} className="bg-[#1C1E24]/95 rounded-t-2xl p-3 pb-safe max-h-[40dvh] overflow-y-auto no-scrollbar shadow-2xl w-full relative z-[99999]"> 
      <div className="flex justify-between items-center mb-3"> 
        <h3 className="text-white font-semibold text-sm flex items-center gap-2"> 
          <Gift className="text-white" size={16} /> 
          Send Gift 
        </h3> 
        <div className="flex items-center gap-2 bg-[#13151A] px-2.5 py-0.5 rounded-full border border-secondary/20"> 
      ...
      <div className="flex items-center gap-4 mb-2 px-1"> 
        <button 
            className={`text-xs font-medium pb-1.5 transition-colors relative ${activeTab === 'small' ? 'text-white' : 'text-white/50 hover:text-white/80'}`} 
            onClick={() => setActiveTab('small')} 
        > 
            Small Gift 
            {activeTab === 'small' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[#C9A96E] rounded-t-full" />} 
        </button> 
        <button 
            className={`text-xs font-medium pb-1.5 transition-colors relative ${activeTab === 'exclusive' ? 'text-white' : 'text-white/50 hover:text-white/80'}`} 
              ...
              <div className="grid grid-cols-4 gap-2"> 
                ...
                > 
                  ...
                  <div className="text-center z-10"> 
                    <p className="text-[10px] text-white font-medium truncate w-full mb-0.5 group-hover:text-white"> 
                      {universeGift.name} 
                    </p> 
            ...
            <div className="grid grid-cols-4 gap-2"> 
              {bigGifts.map((gift) => ( 
                ...
                > 
                  ...
                  <div className="text-center z-10"> 
                    <p className="text-[10px] text-white font-medium truncate w-full mb-0.5 group-hover:text-white">{gift.name}</p> 
                    <div className="flex items-center justify-center gap-1"> 
                      <Coins size={9} className="text-secondary" /> 
