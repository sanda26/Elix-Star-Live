    ...
    <div className="min-h-[100dvh] bg-[#121212] text-[#00f2ea] flex justify-center px-2">
      <div className="w-full max-w-[480px] h-[100dvh] rounded-3xl overflow-hidden bg-[#121212] flex flex-col pt-[var(--safe-top)] pb-[calc(var(--safe-bottom)+12mm)] overflow-y-auto p-4">
        ...
        {savedAccounts.length > 0 && (
          <div className="mb-6">
            <h3 className="text-[9px] text-[#00f2ea]/40 uppercase tracking-widest font-bold mb-2 pl-1">Switch Accounts</h3>
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/10 px-1">
              {savedAccounts.map((acc) => {
                ...
                return (
                  ...
                  >
                    <div className={`relative w-10 h-10 rounded-full p-[1.5px] ${isActive ? 'bg-gradient-to-tr from-[#00f2ea] to-yellow-400' : 'bg-white/10 group-hover:bg-white/30'} transition-all`}>
                      <img 
                      ...
                      />
                      {isActive && (
                        <div className="absolute bottom-0 right-0 w-3 h-3 bg-[#00f2ea] rounded-full border-[1.5px] border-black flex items-center justify-center">
                      ...
                      {!isActive && (
                        <button
                          ...
                          className="absolute -top-1 -right-1 w-4 h-4 bg-red-500/80 rounded-full text-[#00f2ea] text-[8px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                          ×
                        </button>
                      )}
                    ...
                    <div className="text-center w-full">
                      <p className={`text-[9px] font-medium truncate w-full ${isActive ? 'text-[#00f2ea]' : 'text-[#00f2ea]'}`}>
                        {acc.username}
                      </p>
                    </div>
                  </div>
              ...
              >
                <div className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-white/10 transition-colors">
                  <span className="text-lg text-[#00f2ea]/50 font-light">+</span>
                </div>
                <div className="text-center">
                  <p className="text-[9px] font-medium text-[#00f2ea]/50">Add</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mb-4 hidden">
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-sm font-semibold border transition ${
                mode === 'signin'
                  ? 'bg-[#00f2ea] text-black border-[#00f2ea]'
                  : 'bg-transparent5 text-[#00f2ea] border-white/10 hover:bg-transparent10'
              }`}
            >
              Sign in
            </button>
        ...
        {!user && (
          <form onSubmit={onSubmit} className="space-y-4 mb-6 max-w-[90%] mx-auto">
            <div className="space-y-1.5">
              <label className="text-[10px] text-[#00f2ea]/50 font-medium uppercase tracking-wider pl-1">Email</label>
              <div className="relative group">
                <div className="absolute inset-0 bg-gradient-to-r from-[#00f2ea]/20 to-[#00f2ea]/5 rounded-xl opacity-0 group-focus-within:opacity-100 transition-opacity duration-300" />
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#00f2ea]/40 group-focus-within:text-[#00f2ea] transition-colors" />
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full bg-[#121212] border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm text-[#00f2ea] placeholder-[#00f2ea]/20 outline-none focus:border-[#00f2ea]/50 transition-all"
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] text-[#00f2ea]/50 font-medium uppercase tracking-wider pl-1">Password</label>
              <div className="relative group">
                <div className="absolute inset-0 bg-gradient-to-r from-[#00f2ea]/20 to-[#00f2ea]/5 rounded-xl opacity-0 group-focus-within:opacity-100 transition-opacity duration-300" />
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#00f2ea]/40 group-focus-within:text-[#00f2ea] transition-colors" />
                <input
                  ...
                  className="w-full bg-[#121212] border border-white/10 rounded-xl pl-10 pr-10 py-3 text-sm text-[#00f2ea] placeholder-[#00f2ea]/20 outline-none focus:border-[#00f2ea]/50 transition-all"
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#00f2ea]/30 hover:text-[#00f2ea] transition-colors"
                >
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex items-center">
                <input
                  ...
                  className="peer h-4 w-4 rounded-full border border-white/30 bg-transparent appearance-none checked:border-[#00f2ea] checked:bg-[#00f2ea] transition-all cursor-pointer"
                />
              ...
              <label htmlFor="save-login" className="text-xs text-[#00f2ea]/60 cursor-pointer select-none">Save login info</label>
            </div>

            {error && <div className="text-xs text-rose-300">{error}</div>}
            {info && <div className="text-xs text-[#00f2ea]/70">{info}</div>}
            ...
            <button
              ...
              className="w-full bg-[#00f2ea] text-black font-bold rounded-xl py-3 text-sm disabled:opacity-60 shadow-[0_0_15px_rgba(230,179,106,0.3)] hover:shadow-[0_0_20px_rgba(230,179,106,0.5)] transition-all active:scale-[0.98]"
            >
        ...
        {user && (
          <div className="space-y-4 mb-6 max-w-[90%] mx-auto">
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="save-login-user" className="text-xs text-[#00f2ea]/60 cursor-pointer select-none">Save login info</label>
            </div>
