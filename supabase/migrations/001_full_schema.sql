...
DROP POLICY IF EXISTS "live_streams_insert_auth" ON live_streams;
CREATE POLICY "live_streams_insert_auth" ON live_streams FOR INSERT WITH CHECK (auth.uid() = user_id);
...
DROP POLICY IF EXISTS "live_viewers_select" ON live_viewers;
...
CREATE POLICY "live_viewers_insert_auth" ON live_viewers FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "live_viewers_update_own" ON live_viewers;
CREATE POLICY "live_viewers_update_own" ON live_viewers FOR UPDATE USING (auth.uid() = user_id);
...
DROP POLICY IF EXISTS "battle_invites_select" ON battle_invites;
CREATE POLICY "battle_invites_select" ON battle_invites FOR SELECT USING (auth.uid() = from_user OR auth.uid() = to_user);
DROP POLICY IF EXISTS "battle_invites_insert" ON battle_invites;
CREATE POLICY "battle_invites_insert" ON battle_invites FOR INSERT WITH CHECK (auth.uid() = from_user);
DROP POLICY IF EXISTS "battle_invites_update" ON battle_invites;
CREATE POLICY "battle_invites_update" ON battle_invites FOR UPDATE USING (auth.uid() = from_user OR auth.uid() = to_user);
...
CREATE TABLE IF NOT EXISTS battles (
  ...
  host_b     uuid REFERENCES auth.users(id) ON DELETE CASCADE,
  status     text DEFAULT 'pending',
...
CREATE POLICY "battles_insert" ON battles FOR INSERT WITH CHECK (auth.uid() = host_a);
DROP POLICY IF EXISTS "battles_update" ON battles;
CREATE POLICY "battles_update" ON battles FOR UPDATE USING (auth.uid() = host_a OR auth.uid() = host_b);
...
CREATE POLICY "live_chat_messages_select" ON live_chat_messages FOR SELECT USING (true);
DROP POLICY IF EXISTS "live_chat_messages_insert" ON live_chat_messages;
CREATE POLICY "live_chat_messages_insert" ON live_chat_messages FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_live_chat_live ON live_chat_messages(live_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_live_events_live ON live_events(live_id, created_at DESC);
